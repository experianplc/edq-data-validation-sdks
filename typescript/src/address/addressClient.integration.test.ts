...
import { ValidateMatchConfidence } from './validate/validateMatchConfidence';
import { ValidateMatchType } from './validate/validateMatchType';
...
describe('Address client tests', async () => {
    ...
    test(`Lookup with no datasets throws`, async () => {
        const config = new AddressConfiguration(
            ...
            const client = new AddressClient(config);
            await expect(client.lookup("SW1E 5JL", LookupType.PostalCode)).rejects.toThrow("No datasets have been supplied in the configuration.");        
    ...
    test(`Lookup with datasets from different countries throws`, async () => {
        const config = new AddressConfiguration(
            ...
            const client = new AddressClient(config);
            await expect(client.lookup("SW1E 5JL", LookupType.PostalCode)).rejects.toThrow("All datasets must belong to the same country.");        
    ...
    test(`Lookup with no options`, async () => {
        ...
        const client = new AddressClient(config);
        const result = await client.lookup("SW1E 5JL", LookupType.PostalCode);
    ...
    test(`Lookup with options`, async () => {
        ...
        let result = await client.lookup("SW1E 5", LookupType.PostalCode);
        expect(result.confidence).toEqual("NoMatches");
        ...
        result = await client.lookup("SW1E 5", LookupType.PostalCode);
        ...
        // config.options.formatLayoutName = "ElectricityUtilityLookup";
        // client = new AddressClient(config);
    ...
    test(`Validate address. Multiple addresses in response`, async () => {
        ...
        const client = new AddressClient(config);
        const result = await client.validate(["Main Ave, Lidcombe, 2141"]);
